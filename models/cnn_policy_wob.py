...
class CNNPolicyWoB(BasePolicy):
    def __init__(self, sess, input_shape, num_actions, reuse=False, is_training=True, name='train'):
        ...
        with tf.variable_scope("policy", reuse=reuse):
            conv1 = conv2d('conv1', tf.cast(self.X_input, tf.float32), num_filters=32, kernel_size=(4,4),
                           padding='SAME', stride=(1, 1), initializer=orthogonal_initializer(np.sqrt(2)),
            ...
            conv2 = conv2d('conv2', conv1, num_filters=64, kernel_size=(4, 4), padding='SAME', stride=(1, 1),
            ...
            fc3 = dense('fc3', conv2_flattened, output_dim=64, initializer=orthogonal_initializer(np.sqrt(2)),
                        activation=tf.nn.relu, is_training=is_training,
                        batchnorm_enabled=True,
                        dropout_keep_prob=0.7
                        )

            self.policy_logits = dense('policy_logits', fc3, output_dim=num_actions,
                                       initializer=orthogonal_initializer(np.sqrt(1.0)), is_training=is_training)

            self.value_function = dense('value_function', fc3, output_dim=1,
                                        initializer=orthogonal_initializer(np.sqrt(1.0)), is_training=is_training)
